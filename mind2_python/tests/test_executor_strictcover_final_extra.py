# mind2_python/tests/test_executor_strictcover_final_extra.py
import types
import pytest
...
def make_executor(monkeypatch, **kwargs):
    ...
    mt5 = types.SimpleNamespace(
        ...
        symbol_info=lambda s: types.SimpleNamespace(volume_min=0.1, volume_max=5.0, volume_step=0.1, digits=5),
...
def test_connect_fail_twice(monkeypatch):
    """_connect fail both times -> raise RuntimeError"""
    monkeypatch.setenv("MT5_LOGIN", "1")
    monkeypatch.setenv("MT5_PASSWORD", "p")
    monkeypatch.setenv("MT5_SERVER", "s")
    monkeypatch.setenv("MT5_PATH", "path")
...
def test_shutdown(monkeypatch):
    ...
    e.shutdown()  # should log and not raise


def test_execute_multi_entry_order_fail(monkeypatch):
    """multi-entry margin pass but order_send retcode != DONE"""
    def bad_order_send(req): return DummyResult(retcode=999, comment="bad")
    e = make_executor(monkeypatch, order_send=bad_order_send)
    dec = {
        "symbol": "X",
        "decision": "BUY",
        "exit_levels": {
            "entries": {
                "1": {"lot": 0.2, "sl": 9.0, "tp": [{"price": 12.3}]}
            }
        }
    }
    # margin pass but retcode fail -> should return None
    out = e.execute(dec)
    assert out is None


def test_close_position_retcode_fail(monkeypatch):
    """close_position order_send retcode != DONE"""
    def bad_order_send(req): return DummyResult(retcode=999, comment="bad")
    e = make_executor(monkeypatch, order_send=bad_order_send,
