# mind2_python/tests/test_executor_strictcover_final_killshot.py
import types
import pytest
...
def test_connect_fail_twice_full(monkeypatch, capsys):
    """_connect fail 2 รอบ -> cover 89–92"""
    monkeypatch.setenv("MT5_LOGIN", "1")
    monkeypatch.setenv("MT5_PASSWORD", "p")
    monkeypatch.setenv("MT5_SERVER", "s")
    monkeypatch.setenv("MT5_PATH", "path")
    ...
    assert "MT5 initialize failed" in out or "retrying" in out


def test_shutdown_logs(monkeypatch, capsys):
...
def test_execute_multi_entry_continue(monkeypatch, capsys):
    ...
    out = e.execute(dec)
    assert out is None
    outcap = capsys.readouterr().out
    assert "Margin not enough" in outcap


def test_close_position_retcode_fail(monkeypatch, capsys):
    ...
    assert out is None
    outcap = capsys.readouterr().out
    assert "Order failed" in outcap or "❌" in outcap
