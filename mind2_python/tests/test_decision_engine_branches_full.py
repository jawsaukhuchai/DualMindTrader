...
# --- fusion_decision tie fallback (ai_res missing "decision") ---
def test_fusion_decision_tie_fallback():
    ai_res = {}  # no decision key
...
class DummyExitNoKeys:
    def calc(self, *a, **k):
        return {"entries": {}}  # no sl/tp keys at all

def test_process_no_sl_tp_fallback(base_engine, caplog):
    entry = DummyTradeEntry()
